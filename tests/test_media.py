...
def test_get_file_info():
    ...
    next(gen)
    next(gen)
    assert media_list.get_file_info(0, 'filename') == 'Dad.mpg'


def test_running_file_name():
    ...
    assert media_list.running_file_name() == 'Dad'


def test_running_file_info():
    media_list = MediaList(profile=profile)
    gen = media_list.populate(('Dad.mpg',))
    next(gen)
    next(gen)
    media_list.position = 0
    assert media_list.running_file_info('filename') == 'Dad.mpg'


def test_running_file_output_name():
    """Test MediaList.running_file_output_name()."""
    media_list = MediaList(profile=profile)
    gen = media_list.populate(('Dad.mpg',))
    next(gen)
    next(gen)
    assert media_list.running_file_output_name('.', False) == 'Dad.mpg'
...
def test_is_exhausted_true():
    ...
    next(gen)
    media_list.position = 0
    assert media_list.is_exhausted
...
def test_is_exhausted_false():
    ...
    next(gen)
    next(gen)
    media_list.position = -1
    assert not media_list.is_exhausted
...
def test_all_stopped_true():
    ...
    next(gen)
    next(gen)
    media_list.set_file_status(0, 'Stopped')
    assert media_list.all_stopped
...
def test_all_stopped_false():
    ...
    next(gen)
    next(gen)
    media_list.set_file_status(0, 'Todo')
    assert not media_list.all_stopped
